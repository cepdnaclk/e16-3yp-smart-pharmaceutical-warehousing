        ...
        for j in i:
            ...
            # if the path is not stored
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future =  executor.submit(mp.dijkstra, graph,k, j, shortestPathtoNodes["{}-{}".format(k, j)], [], {}, {})
...
def getPath(graph, src, shortestPathtoNodes,OptRoute):
    ...
    # checks if a path exists in the calculated shortest path list, from src node to dest through given route
    # if NOT, calculate the path and add it to the shortest path list
    if ("{}-{}".format(src, OptRoute[0])) not in shortestPathtoNodes.keys():
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future =  executor.submit(mp.dijkstra, graph, src, OptRoute[0], shortestPathtoNodes["{}-{}".format(src, OptRoute[0])], [], {}, {})
            
        #print(shortestPathtoNodes["{}-{}".format(src, OptRoute[0])])
        bot_path[0] = shortestPathtoNodes["{}-{}".format(src, OptRoute[0])]
    
    # if yes, obtain the path from the shortest path list and add it to bot_path dict
    else:
    ...
    # checks if a shortest path from the last rack to the src exists
    # # if NOT calculate it and add it to the path  
    if ("{}-{}".format(OptRoute[-1], src)) not in shortestPathtoNodes.keys():
        ...
        # calculates the shortest path
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future =  executor.submit(mp.dijkstra, graph, OptRoute[-1], src, shortestPathtoNodes["{}-{}".format(OptRoute[-1], src)], [], {}, {})
    ...
    # if exists, add it to the bot_path list
    else:
        # path from src to the first rack
        bot_path[-1] = shortestPathtoNodes["{}-{}".format(OptRoute[-1], src)]

    #print(bot_path)
    # return bot_path list
    return bot_path
